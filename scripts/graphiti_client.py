...
class GraphitiClient:
    ...
    def __init__(
        ...
        try:
            ...
            # Note: build_indices_and_constraints() must be called separately
            # via async initialize() method to avoid event loop conflicts
    ...
    async def initialize(self):
        """
        Async initialization step for building indices and constraints.

        Must be called after __init__ when using in an async context to avoid
        event loop conflicts. In sync contexts (like Streamlit), this can be
        skipped as indices will be built on first operation.
        """
        try:
